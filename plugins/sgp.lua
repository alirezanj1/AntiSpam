	...
	for k,v in pairs(result) do
		...
		elseif memberid and vusername ~= member and vpeer_id ~= memberid then
			...
			if group_owner then
				...
				text = "["..memberid.."] Now IS Owner Of This Group!"
			end
		end
	end
 end
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'link' then
			...
			if not group_link then
				return "This Group Doesnt Have Link!\nSend /newlink For CReate Link Or If Im Not Creator Send /setlink Then Send Group Link"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested group link ["..group_link.."]")
			return "SuperGroup Link With Https\n"..group_link
		end
		...
		if matches[1] == "setabout" and is_momod(msg) then
			...
			return "Description has been set for this supergroup.\nYou can top to chat & see it!"
		end
		...
		if matches[1] == 'setphoto' and is_momod(msg) then
			...
			return 'Now, Send Me New SuperGroup Photo!'
		end
		...
		if matches[1] == 'clean' then
			...
			if matches[2] == 'about' then
				...
				local data_cat = 'description'
				if data[tostring(msg.to.id)][data_cat] == nil then
					return 'No About!Set About & Try Again'
				end
				data[tostring(msg.to.id)][data_cat] = nil
				save_data(_config.moderation.data, data)
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] cleaned about")
				channel_set_about(receiver, about_text, ok_cb, false)
				return "SuperGroup About TExt Cleaned !"
			end
		...
		if matches[1] == 'help' and not is_owner(msg) then
			text = "This command not work in supergroups!Send /superhelp in this group or in private"
